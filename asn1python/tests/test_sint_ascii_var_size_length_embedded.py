...
def _encode_and_decode_signed_integer_as_ascii(acn_encoder: ACNEncoder, input_number: int) -> tuple[bool, int]:
    """Helper function to encode and decode a signed integer value as ascii.

    Returns:
        Tuple of (success, decoded_value)
    """
    encoded_res = acn_encoder.enc_sint_ascii_var_size_length_embedded(input_number)
    if not encoded_res.success:
        return False, 0

    acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    decoded_res = acn_decoder.dec_sint_ascii_var_size_length_embedded()
...
def _encode_and_decode_multiple_signed_integers_as_ascii(acn_encoder: ACNEncoder, input_numbers: list[int]) -> tuple[bool, list[int]]:
...
def _test_single_signed_integer(acn_encoder: ACNEncoder, input_number: int) -> None:
    """Helper function to test encoding/decoding of a single positive integer value."""
    success, decoded_value = _encode_and_decode_signed_integer_as_ascii(acn_encoder, input_number)
...
def _test_multiple_signed_integers(acn_encoder: ACNEncoder, input_numbers: list[int]) -> None:
    """Helper function to test encoding/decoding of multiple positive integer values."""
    success, decoded_values = _encode_and_decode_multiple_signed_integers_as_ascii(acn_encoder, input_numbers)
...
def test_enc_dec_sint_ascii_const_size_single_value(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    ...
    _test_single_signed_integer(acn_encoder, input_number)
...
def test_enc_dec_sint_ascii_const_size_multiple_values(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    ...
    _test_multiple_signed_integers(acn_encoder, input_numbers)

def test_enc_dec_sint_ascii_const_size_zero(acn_encoder: ACNEncoder, seed: int) -> None:
    input_number: int = 0
    _test_single_signed_integer(acn_encoder, input_number)

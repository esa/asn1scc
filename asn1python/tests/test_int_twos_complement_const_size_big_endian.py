...
def _encode_and_decode_single_twos_complement(acn_encoder: ACNEncoder, input_number: int, bit: int) -> tuple[bool, int]:
    ...
    """
    encoded_res = acn_encoder.enc_int_twos_complement_const_size_big_endian(input_number, bit)
    if not encoded_res.success:
        return False, 0

    acn_decoder: ACNDecoder = acn_encoder.get_decoder()
    decoded_res = acn_decoder.dec_int_twos_complement_const_size_big_endian(bit)
...
def test_enc_dec_int_twos_complement_const_size_big_endian_single_value(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    input_number: int = get_random_signed(byte * 8)
    _test_single_twos_complement(acn_encoder, input_number, byte * 8)


def test_enc_dec_int_twos_complement_const_size_big_endian_multiple_values(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
...
def test_enc_dec_int_twos_complement_const_size_big_endian_zero(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    input_number: int = 0
    _test_single_twos_complement(acn_encoder, input_number, byte * 8)


def test_enc_dec_int_twos_complement_const_size_big_endian_max_value(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    input_numbers: list[int] = [get_signed_max(byte * 8), get_signed_min(byte * 8)]
    _test_multiple_twos_complement(acn_encoder, input_numbers, byte * 8)


def test_enc_dec_int_twos_complement_const_size_big_endian_exceed_max_value(acn_encoder: ACNEncoder, seed: int, byte: int) -> None:
    input_number: int = get_signed_max(byte * 8) + 1000
    encoded_res = acn_encoder.enc_int_twos_complement_const_size_big_endian(input_number, byte * 8)
